...
# Seniority filter: reject leadership / senior-LEVEL titles by keyword.
# Role TYPES (SWE, network, solutions, data, ML, platform, infra, ...) are never
# filtered here — only the level. Whole-word, case-insensitive.
SENIORITY_TITLE_RE = re.compile(
    r"\b(principal|staff|senior|sr|snr|lead|leader|manager|director|"
    r"vp|vice\s+president|head\s+of|distinguished|fellow)\b",
    re.IGNORECASE,
)


def is_senior_title(title: str) -> bool:
    return bool(SENIORITY_TITLE_RE.search(title or ""))
...
def main() -> None:
    ...
    # Safety filter: only include jobs requiring <= 2 years of experience
    # (new grad / entry level: min_years 0, 1, or 2). Archit, not the roommate.
    jobs = [j for j in jobs if int(j.get("min_years", 0) or 0) <= 2]

    # Seniority filter: drop leadership / senior-level titles (Principal, Staff,
    # Senior, Sr, Lead, Manager, Director, VP, Head of, ...). Role types are kept.
    dropped = [j.get("job_title") for j in jobs if is_senior_title(str(j.get("job_title", "")))]
    if dropped:
        LOG.info("seniority_filtered count=%d titles=%s", len(dropped), dropped)
    jobs = [j for j in jobs if not is_senior_title(str(j.get("job_title", "")))]
